...
if [ $(dpkg-query -W -f='${STATUS}' $1 2>/dev/null | grep -c "ok installed") -eq 1 ]; then
    ...
    {
        ConfigurePiAware
    } > >(whiptail --gauge "Configuring PiAware to send MLAT data to TheAirTraffic..." 6 65 0)
fi

# Setup the Netcat script.
SetupNetcat > >(whiptail --title "TheAirTraffic Setup Script" --gauge "Setting up and executing the TheAirTraffic Netcat script..." 6 65 0)
...
whiptail --title "TheAirTraffic Setup Script" --msgbox "$THANKS" 17 65
